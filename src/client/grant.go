...
func (s Grant) Grants(m *ice.Message, arg ...string) {
	_sql_exec(m.Spawn(), s.meta(m, m.Option(aaa.SESS), MYSQL), kit.Format("create user '%s'@'%s' identified by '%s'",
		m.Option(aaa.USERNAME), m.Option(tcp.HOST), m.Option(aaa.PASSWORD)))
	_sql_exec(m.Spawn(), s.meta(m, m.Option(aaa.SESS), MYSQL), kit.Format("grant %s on %s to '%s'@'%s'",
