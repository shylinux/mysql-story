...
type server struct {
	...
	start  string `name:"start port*=10002 username*=demo password*=demo" help:"启动"`
...
func (s server) Start(m *ice.Message, arg ...string) {
	s.Code.Start(m, "", "bin/mysqld", "--basedir=./", "--datadir=./data", "--plugin-dir=./lib/plugin",
		"--socket=mysqld.socket", "--log-error=mysqld.log", "--pid-file=mysqld.pid", func(p string) []string {
